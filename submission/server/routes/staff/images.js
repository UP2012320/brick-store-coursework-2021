...
const onUploadRequest = (request, reply, done) => {
  // eslint-disable-next-line require-atomic-updates
  request.buffer = Buffer.from('');
  done();
};
const validateUpload = async (request, reply) => {
...
const saveImage = async (newId, buffer) => {
...
export const deleteImageFromDatabase = async (client, id) => {
  const [imageExists, imageExistsError] = await sendQuery(client,
  ...
  const [, imagesError] = await sendQuery(client, 'DELETE FROM images WHERE image_id = $1', [id]);
  return !imagesError;
};
...
const images = async (fastify, options) => {
  ...
  fastify.delete('/:id', {schema: deleteImageSchema}, async (request, reply) => {
