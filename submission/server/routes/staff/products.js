...
const getTypeAndColourId = async (client, type, colour) => {
  let colourId;
  let typeId;
  if (colour) {
    const lowerColour = colour.toLowerCase().trim();
    const [colourCheckResult] = await sendQuery(client,
    ...
    if (colourCheckResult) {
      if (colourCheckResult.rows.length === 0) {
        const [colourInsertResult] = await sendQuery(client,
  ...
  if (type) {
    ...
    const [typeCheckResult] = await sendQuery(client, 'SELECT * FROM brick_types WHERE type_name = $1', [lowerProduct]);
    if (typeCheckResult) {
      if (typeCheckResult.rows.length === 0) {
        const [typeInsertResult] = await sendQuery(client,
...
const products = async (fastify, options) => {
  fastify.addHook('preHandler', async (request, reply) => {
    validatePermissions(request, reply, ['write:products', 'update:products', 'delete:products']);
  });
  fastify.post('/', {schema: newProductSchema}, async (request, reply) => {
    ...
    const result = await fastify.pg.transact(async (client) => {
      ...
      const [, inventoryInsertError] = await sendQuery(client,
        `
          INSERT INTO inventory (inventory_id, item_name, slug, description, colour, type, price, discount, stock,
                                 date_added, visible)
      ...
      if (product.images) {
        for (const image of product.images) {
          const [, imageError] = await sendQuery(client, `INSERT INTO inventory_images (inventory_id, image_id)
                                                          VALUES ($1, $2)`, [id, image]);
          if (imageError) {
  ...
  fastify.put('/', {schema: updateProductSchema}, async (request, reply) => {
    const {product} = request.body;
    const [result, error] = await sendQuery(fastify.pg.pool, `SELECT * FROM inventory
             WHERE inventory.inventory_id = $1`, [product.inventory_id]);
    if (error || !result) {
    ...
    const updateResult = await fastify.pg.transact(async (client) => {
      ...
      const [existingImages, existingImagesError] = await sendQuery(client, `SELECT image_id as "imageId", inventory_id as "inventoryId" FROM inventory_images
             WHERE inventory_id = $1`, [product.inventory_id]);
      ...
      if (existingImages?.rows && product.images) {
        ...
        const newImages = product.images.filter((image) => !existingImages.rows.some((existingImage) => existingImage.imageId ===
          image));
        for (const image of newImages) {
          const [, imageError] = await sendQuery(client, `INSERT INTO inventory_images (inventory_id, image_id)
                                                          VALUES ($1, $2)
                                                          RETURNING *`, [product.inventory_id, image]);
          if (imageError) {
  ...
  fastify.delete('/:id', {schema: deleteProductSchema}, async (request, reply) => {
    const {id} = request.params;
    const result = await fastify.pg.transact(async (client) => {
      const [existingImages, existingImagesError] = await sendQuery(client, `SELECT * FROM inventory_images
