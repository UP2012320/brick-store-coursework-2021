...
export default function createProduct(props) {
  ...
  if (productDetails) {
    ...
    if (productDetails.images) {
      ProductImage = createElement('img',
        {src: getImageUrl(productDetails.images[0])},
        productStyles.productImageContainer
      );
    }
    const ProductDetailsContainer = createElement('div', undefined, productStyles.productDetailsContainer);
    const ProductDetailsTitle = createElement('h1',
      {textContent: productDetails.name},
      productStyles.productDetailsTitle
    );
    const ProductDetailsId = createElement('p',
      {textContent: productDetails.inventory_id.toUpperCase()},
      productStyles.productDetailsId
    );
    const ProductDetailsStock = createElement('p',
      {textContent: `${productDetails.stock} in Stock`},
      productStyles.productDetailsStock
    );
    const ProductDetailsDescriptionContainer = createElement('div',
      undefined,
      productStyles.productDetailsDescriptionContainer
    );
    const ProductDetailsDescriptionHeader = createElement('h1',
      {textContent: 'Details:'},
      productStyles.productDetailsDescriptionHeader
    );
    const ProductDetailsDescription = createElement('p',
      {textContent: productDetails.description},
      productStyles.productDetailsDescription
    );
    const ProductDetailsPrice = createElement('p',
      {textContent: formatPrice(productDetails.price)},
      productStyles.productDetailsPrice
    );
