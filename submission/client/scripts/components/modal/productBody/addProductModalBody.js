 ...
 * discount
 */
const deleteImagesFromServer = async (...imageIds) => {
...
export default function modifyProductModalBody(props) {
  ...
  const [submitMessage, setSubmitMessage] = useState(props.key, {error: false, message: ''});
  const newImages = useRef(props.key, []);
  useEffect(props.key, () => {
  }, [props.closed]);
