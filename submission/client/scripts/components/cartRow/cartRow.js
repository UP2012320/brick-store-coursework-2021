...
import {formatPrice, getImageUrl, getProductUrl, nameof} from 'Scripts/helpers';
...
export default function createCartRow(props) {
  props.key ??= nameof(createCartRow);
  const quantityDebounceTimeout = useRef(props.key, undefined);
  const cartRow = createElement('div', undefined, cartStyles.cartRow);
  cartRow.setAttribute('key', props.key);
  const cartImageContainer = createElement('a',
    {href: getProductUrl(props.cartItem.product.slug)},
    cartStyles.cartImgContainer
  );
  registerLinkClickHandler(cartImageContainer);
  const cartImage = createElement('img',
    {src: props.cartItem.product.images ? getImageUrl(props.cartItem.product.images[0]) : ''},
    cartStyles.cartImg
  );
  const cartTitle = createElement('a',
    {href: getProductUrl(props.cartItem.product.slug), textContent: props.cartItem.product.name},
  ...
  if (props.outOfStock) {
    outOfStock = createElement('p',
      {
        textContent: props.stockOnHand === 0
          ? 'Sorry, this item is out of stock'
          : `Sorry, there is only ${props.stockOnHand} of this item available`
      },
  ...
  const cartQuantity = createElement('input', {
    ...
    onchange: (event) => {
      ...
      quantityDebounceTimeout.current = window.setTimeout(() => {
        if (target.value === '0') {
