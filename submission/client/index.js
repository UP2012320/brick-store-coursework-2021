...
import {
  clearUseEffect,
  fireUseEffectDiscardQueue,
  fireUseEffectQueue,
  resetUseEffectStateIndexes
} from 'Scripts/hooks/useEffect';
...
import createBrowse from 'Scripts/pages/browse/browse';
...
const render = async () => {
  ...
  switch (targetedRoute) {
    case 'browse':
      body = createBrowse(queryStrings);
      break;
    case 'product':
      body = createProduct({restArgs});
      break;
    case 'cart':
      body = createCart();
      break;
